...
Unlike the *parsing* approach in CLAN which entails the sequential processing of strings, childes-db treats CHILDES as a set of linked tables, with records corresponding to intuitive abstractions such as words, utterances, and transcripts.
...
Although we expect most users to interact with the database through the `childesr` API for the `R` language described below, we begin with a top-level description and motivation for the design of the database schema, details of the database’s current technical implementation, and our versioning scheme.
...
At its core, childes-db is a database consisting of a set of linked tabular data stores where rows correspond to linguistic entities like words, utterances, and sampling units like transcriptions and corpora. 
...
The smallest unit of abstraction tracked by the database is a token, treated here as the citation orthographic form of a word.
...
Other tables in the database comprise a hierarchical system of collections built on top of tokens -- *utterance*, *transcription*, *corpus*, and *collection* -- which stores attributes appropriate for each level of description.
...
We chose to treat participants separate from the above hierarchy because children can occur across transcripts (e.g., the Brown Corpus).
A participant identifier is associated with every word and utterance, including a name, role, 3-letter CHILDES identifier (CHI, MOT, FAT, etc.), and the range of ages (or age of corresponding child) for which they are observed.
For non-child participants (caregivers and others), the record additionally contains an identifier for the corresponding target child to associate caregivers and children.

## Technical implementation

The above tabular datastore is implemented as a MySQL database.
As an industry-standard, open-source relational database server, the above database can be accessed directly from a wide range of programming languages.
...
We parse the XML (eXtensible Markup Language) release of CHILDES hosted by \url{childes.talkbank.org} using the NLTK library in Python.
<!-- citation for NLTK -->
...
This logic includes decisions regarding what information sources are captured in the current release of the database, and which were left for future development (detailed below).

<!-- testing details -->
The data imported into childes-db requires extensive data integrity checks to ensure that our parse of the corpora is accurate and preferable over ad hoc parsers developed by many individual researchers.
In order to evaluate this, we compared unigram counts in our database with those outputted by CLAN, the command-line tool built specifically for analysis of transcripts coded in CHAT.
...
Here we introduce a simple versioning system by adding a new complete parse of the current state of CHILDES every six months.
...
A child's lexical statistics has long been an object of study in child language acquisition research. Namely, the frequency counts of various words may provide insight into the cognitive, conceptual, and linguistic experience of a young child. In this web app, we provide users the ability to search for any word spoken by a participant in the CHILDES corpora and track the usage of that word by a child or caregiver over time. Because of the various toggles available to the user that can subset the data, a user may display frequency curves for a single child in the Brown corpus or all Spanish speaking children, if desired. In addition, users may plot frequency curves belonging to caregivers alongside their child for convenient side-by-side comparisons. A single word or multiple words may be entered into the input box, separated only by a comma. <!-- cite original ChildFreq paper as inspiration? -->

<!-- figure of freq app -->

### Derived Measures

Researchers in child language acquisition have often assessed a child's linguistic performance by extracting various measures from their speech. These developmental metrics have also had a major application in speech-language pathology (SLP), where subpar measures may be indicative of speech, language, or communication disorders. The Derived Measures web app is built to provide the most common measures typically extracted from children's speech. The most famous of these measures, mean length of utterance (MLU), is used to characterize the linguistic productivity of children and is available through this web app. However, only MLU-w (mean length of utterance in words) is provided which has been shown to correlate with MLU-m (mean length of utterance in morphemes) in the English language <!-- citation needed -->. The other measures provided in this app are measures of lexical diversity: TTR (type-token ratio), MTLD (measure of textual lexical diversity) and HD-D (hypergeometric distribution of diversity) <!-- cite hd-d? mccarthy jarvis 2007 -->. As with the Frequency Counts app, a user may subset the data as he or she chooses, compare measures between caregivers and children, and aggregate across children from different corpora.

<!-- figure of derived measures app -->

### Population Viewer

Many times a researcher will want to investigate the statistics of a given corpus (i.e the size, number of utterances, number of tokens, etc.) before choosing a target corpus for their analysis. This web app is intended to provide a basic overview of various corpora in the CHILDES ecosystem, facilitating familiarity with the different corpora and thus assisting with the overall analytic end-to-end process. <!-- note that the graphs don't currently include a grouping factor, so adding multiple corpora generates one line, so you can't actually compare two corpora in the same visualization -->

<!-- figure of population viewer app -->
