...
abstract: |
  The Child Language Data Exchange System (CHILDES) has played a critical role in research on child language development. Access to these data can be complex for novices and difficult to automate for advanced users, however. To address these issues, we introduce childes-db, a database-formatted mirror of CHILDES that improves data accessibility and usability by offering novel interfaces, including browsable web applications and an R application programming interface (API). Along with a versioned infrastructure that facilitates reproducibility of past analyses, these interfaces lower barriers to analyzing naturalistic parent-child language, allowing for a wider range of researchers in language and cognitive development to easily leverage CHILDES in their work. 
...
Gender differences in language production have been found in multiple studies, notably in @eriksson2012differences, a large-scale study that spanned ten languages. A researcher or student in child language acquisition may now investigate this difference with relative ease using `childesr`. By pulling data from the `transcript_by_speaker` table, a user has access to the the multitude of derived linguistic measures that are often used to evaluate a child's linguistic performance. In this worked example we walk through a sample analysis that probes the gender differences in language production.

First, we utilize the `childesr` function call `get_speaker_statistics` to pull data relating to the aforementioned derived measures for children and their transcripts. We exclusively select children and not their caregivers.

```{r echo=TRUE}
stats <- get_speaker_statistics(role = "Target_Child")
```

This call pulls data from all collections and corpora, including those languages for which there are very sparse data. In order to make any substantial inferences from our analysis we begin by filterering languages for which there are a significant number of children. We also filter for children under the age of four years. 
...
Our `transcript_by_speaker` table contains multiple derived measures of language productivity and lexical diversity -- here we used MTLD [@mccarthy2005assessment], a measure that is robust to transcript length, as a way of evaluating children's linguistic productivity. We start by filtering those children for which a sex was defined, who speak a language in our subset, and are in the appropriate age range. We then average their MTLD score across transcripts after having grouped the data by child, age, sex, and language. 
...
The data contained in CHILDES is populated from a diverse array of studies that adhered to varying conditions and paradigms. This point is particularly salient in our gender analysis due to potential non-independence issues that result from longitudinal studies. We chose to fit a model (`mod_data`) of this data that takes these facts into account, and for brevity, is not included here.^[All code and analyses are available at https://github.com/langcog/childes-db-paper] The resultant plot displays the average MTLD scores for various children at different ages, split by gender, with a line corresponding to our model. 
...
This plot reveals a slight gender difference in linguistic productivity in young children, replicating the results of @eriksson2012differences. The goal of this analysis was to showcase an example of the the steps involved in using `childesr` for exploring the CHILDES dataset, as well as some of the potential hazards in working with a diverse set of corpora. 
...
Consider the following demonstration about children's first words. Diary studies and large-scale studies using parent report show that children’s first words tend to fall into a fairly small number of categories: people, food, body parts, clothing, animals, vehicles, toys, household objects, routines, and activities or states [@clark2009first;@fenson1994variability;@tardif2008]. The key insight is that young children talk about what is going on around them: people they see every day, e.g., toys and small household objects they can manipulate or food they can control. To illustrate this point, an instructor could:
