...
How children acquire language is one of the core puzzles in cognitive science, developmental psychology, and linguistics. 
What are the representation that children learn about language, and how to they emerge from the interaction of learning mechanisms and environmental input?
...
Starting with Roger Brown's [-@brown1973] work on Adam, Eve, and Sarah, audio recordings -- and more recently augmented with video recordings -- have been augmented with rich, searchable annotations that have allowed researchers to address a number of questions regarding the language learning environment.
Focusing on language learning in naturalistic contexts also reveals that children have in many cases productive and receptive abilities exceeding those revealed in experimental contexts; owing to challenges in lab-based experiments with young children, often their most revealing and sophisticated uses of language emerge in naturalistic play.

While corpora of early language acquisition are extremely useful, creating such corpora requires significant resources. 
Collecting and transcribing audio and video is costly and extremely time consuming  -- even orthographic transcription (i.e., transcriptions with minimal phonetic detail) can take ten times the length of the original recording to transcribe [@macwhinney2000].
Automated, machine learning -based methods like automatic speech recognition (ASR) have provided only modest gains in efficiency, in that they are limited by the less-than-ideal acoustic properties of home recordings and the poor fit of language models built on adult-directed, adult-produced language samples for characterizing child-directed or child-produced speech.
Thus, researchers' desire for sufficient sample sizes and coverage in analyses of child language corpora can very quickly outstrip their resources. 
<!-- data pooling is necessary) -->

Established in 1984 to address this issue, the Child Language Data Exchange System (CHILDES) aims to make transcripts and recording of children's language available for researchers as free, public datasets [@macwhinney1985; @macwhinney2000; @macwhinney2014].
CHILDES archives tens of thousands of transcripts and associated media across 20+ languages, making it a critical resource for characterizing children’s early productive language use as well as their language environment. 
...
Some studies analyze individual examples in depth [e.g., @snyder2007], other track multiple child-caregiver dyads [e.g., @meylan2017], and still others use the aggregate properties of child or caregiver speech [e.g., @redingtonEtAl1998; @montagEtAl2015].
...
The CHILDES ecosystem typically uses a specialized file format (CHAT), which is stored as plain text but includes structured annotations grouped into tiers (stored on separate lines) so that information about utterances can be stored with accompanying information such as the phonological, morphological, or syntactic structure of the utterance.
These files are usually analyzed using a command-line tool (CLAN), which has a number of specific tools that allow users to count word frequencies, compute statistics (e.g., mean length of utterance, or MLU), and execute complex searches against the data.
While this system is flexible and powerful, mastering the CHAT codes and especially the CLAN tool with its many tools and flags can be daunting for a beginner or student.
...
Third, these parsing scripts are rarely shared or break with subsequent revisions to the dataset, leading to much greater difficulty in reproducing the exact numerical results from papers using CHILDES [see e.g., @meylan2017 for an example].
Fourth, the CHILDES corpus itself is a moving target; computational work using the entire corpus at one time point may include a different set of data than subsequent work due to newly-contributed corpora or revised annotations of existing ones; currently, there is no simple way for researchers to document exactly what version of the corpus has been used short of creating a full mirror of the data.
...
Next, we describe the `childesr` API, which provides a set of R functions for programmatic access to the data.
We conclude by presenting several worked examples of specific uses of the system--both web apps and the R API--for both research and teaching.
...
Many of the other tables in the database are hierarchical collections built out of tokens -- *utterance*, *transcript*, *corpus*, and *collection* -- which store attributes appropriate for each level of description.
...
Participants (e.g., children and caregivers) are represented separately from the token hierarchy because it is common for the same children to appear in multiple transcripts.
...
For non-child participants (caregivers and others), the record additionally contains an identifier for the corresponding target child, such that data correpsonding to children and their caregivers can be easily associated.
...
While the former is appropriate for most users, local installation can provide performance gains (allowing a user to access the database on their machine or on their local network) as well as allowing users to store derived information in the same database.
...
In particular, the CLAN commands FREQ and MLU were used to compare total token counts and mean lengths of utterance for every speaker in every transcript; these numbers were compared to our own using the Pearson correlation coefficient. 
The results of the comparison were .99 and .98 for the unigram count and mlu data, respectively, indicating a reliable parse on our end.
...
The web apps are intended for the casual user who seeks to easy extract developmental indices quickly and without any technical overhead.
...
Because of the various toggles available to the user that can subset the data, a user may display frequency curves for a single child in the Brown corpus or all Spanish speaking children, if desired.
...
There are a number of well-established measures of children's speech that operationalize complexity and diversity.
These developmental metrics have many applications in speech-language pathology (SLP), where measures outside of the normal range may be indicative of speech, language, or communication disorders.
