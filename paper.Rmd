...
abstract: |
  ...
  Access to these data can be both complex for novices and also difficult to automate for advanced users, however.
...
Focusing on language learning in naturalistic contexts also reveals that children have in many cases productive and receptive abilities exceeding those demonstrated in experimental contexts; owing to challenges in lab-based experiments with young children, often their most revealing and sophisticated uses of language emerge in naturalistic play.
...
Thus, researchers' desire for data in analyses of child language corpora can very quickly outstrip their resources. 
...
Third, these parsing scripts are rarely shared -- and when when they are, they typically break with subsequent revisions to the dataset -- leading to much greater difficulty in reproducing the exact numerical results from papers using CHILDES [see e.g., @meylan2017 for an example].
...
A *corpus* consists of one or more transcripts, corresponding to well-known collections like the Brown or Providence corpora.
...
The plot is displayed and updated in real-time, and the underlying data are also available for download alongside the plot to facilitate customization. 
All of these analyses may also be reproduced using the `childesr` package, but the web apps are intended for the casual user who seeks to easy extract developmental indices quickly and without any technical overhead.
...
This web app is intended to provide a basic overview of various corpora in the CHILDES ecosystem, facilitating familiarity with the different corpora and thus assisting with the overall analytic end-to-end process.
...
We also do not have a good sense of the errors of these measurements, are limited to just a few colors at a time before the plot becomes too busy, and cannot combine frequencies across speakers. <!-- SM:"busy" seems a bit informal; MCF: I'm ok with it. :) -->
...
We also restrict ourselves to children from 1--5 years old (12--60 months), and look only at the American English corpora.
...
Gender differences in language production are well-documented. For example, @eriksson2012differences used parent-report data from ten languages and found that female children had larger vocabularies on average than male children in nearly every language.
...
By pulling data from the `transcript_by_speaker` table, users can access derived linguistic measures.
In this worked example we walk through a sample analysis that probes the gender differences in early lexical diversity.
...
This command retrieves data from all collections and corpora, including those languages for which there are very sparse data.
In order to make any substantial inferences from our analysis, we begin by filtering languages for which there are a large number of transcripts ($>500$). 
We also restrict our analysis to children under the age of four years. 
...
Our `transcript_by_speaker` table contains multiple derived measures of lexical diversity -- here we used MTLD [@mccarthy2005assessment], a measure that is relatively robust to transcript length.
...
The goal of this analysis was to showcase an example of the the steps involved in using `childesr` for exploring the CHILDES dataset, as well as some of the potential hazards in working with a diverse set of corpora that were collected through various experimental paradigms. 
