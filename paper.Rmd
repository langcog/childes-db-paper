...
In this worked example we walk through a sample analysis that probes gender differences in early lexical diversity.
...
Our `transcript_by_speaker` table contains multiple derived measures of lexical diversity -- here we used MTLD [@mccarthy2005assessment].
MTLD is derived from the average length of continuous text units above a certain type-token ratio, making it relatively robust to transcript length.
...
One child in particular contained transcripts that were a collection of his most complex utterances (recorded by the parents); this child was excluded from the analysis.
...
data <- stats %>%
  ...
  filter(age_years < max_age, target_child_name != 9387)
```
...
childes-db addresses a number of needs that have emerged in our own research and tecahing, but there are still a number of limitations that point the way to future improvements.
