...
A child's lexical statistics has long been an object of study in child language acquisition research. Namely, the frequency of a word <!-- something important about frequency and development?-->. In this web app, we provide users the ability to search for any word spoken by a participant in the CHILDES corpora and track the usage of that word by a child or caregiver over time. Because of the various toggles available to the user that can subset the data, a user may display frequency curves for a single child in the Brown corpus or all Spanish speaking children if desired. In addition, users may plot frequency curves belonging to caregivers alongside their child for convenient visual comparisons. A single word or multiple words may be entered in the input box, separated only by a comma. 

<!-- figure of freq app -->

### Derived Measures

Characterizing lexical diversity has been another highly utli
