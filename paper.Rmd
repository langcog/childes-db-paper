...
Some studies analyze individual examples in depth [e.g., @snyder2007], others track multiple child-caregiver dyads [e.g., @meylan2017], and still others use the aggregate properties of child or caregiver speech [e.g., @redingtonEtAl1998; @montagEtAl2015].
...
Adopting a tabular format entails trade-offs. Although we have simplified access to the underlying CHILDES dataset through simplified storage mechanisms, the associated metadata may not be preserved without incurring a cost of sparsity. This is due to the relatively unique and idiosyncratic annotations provided in CHAT that only apply to small subsets of utterances, or are only offered by some transcripts.
...
For this reason, we do not preserve all of the metadata available in CHILDES, such as:
