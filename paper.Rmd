...
Yet there are some challenges working with CHILDES, both for students and for advanced. For beginners with fewer technical tools, mastering the system can be tricky. The CHILDES ecosystem uses a special file format (CHAT), which is read by a command-line tool (CLAN) with a host of specific arguments. Mastering the CHAT codes and the CLAN tool can be daunting to navigate for a beginning researcher or student, decreasing the ease of integrating hands-on exploration of the dataset into classes. 

On the opposite end of the spectrum. for data-oriented researchers who are interested in doing large-scale analyses of CHILDES, the current tools are also not ideal. Many researchers who would like to ingest the entire corpus (or some large subset) into a computational analysis must write one-off parsers of the CHAT format to extract the subset of the data they would like to use [EXAMPLES]. This practice is problematic for a number of reasons. Not only is effort wasted in rewriting the same parser again and again, but this process introduces a possible source of error in data handling. Further, these scripta are rarely shared, leading to much greater difficulty in reproducing the exact numerical results from papers using CHILDES [see e.g., @meylan2017]. Finally, the CHILDES corpus itself is a moving target; thus computational work using the entire corpus at one time point may include a different set of data (due to new contributions or revisions). 

Our goal in the current manuscript is to describe a system for extending the functionality of CHILDES to address these issue. childes-db is a database-formatted copy of the CHILDES data that allows programmatic access through an application programming interface (API). This infrastructure allows the creation of web applications for browsing and visualizing the data easily, facilitating classroom use of the dataset. Further, it can be accessed programmatically by advanced researchers, obviating the need to write one-off parsers of the CHAT format. The database is versioned for access to previous images, allowing reproducibility of particular analyses.
