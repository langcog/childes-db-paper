...
We will prioritize the addition of these information sources in response to community feedback; extending coverage to the above information sources will not require changes to the existing database schema.
...
Underneath the hood, each web app is making calls to our `childesr` API and subsequently plotting the data using the popular R plotting package `ggplot2`.
...
The plot is displayed and updated in real-time, and the underlying data are also available for download alongside the plot to facilitate customization. All of these analyses may also be reproduced using the `childesr` package. 
...
<!-- sm: I think we should go ahead and compute this for transcript with MOR tiers? Given that this was one of the first uses, I don't think Brian will like the 'underlying gaps' explanation. MCF: I agree. AS: MLU-m still in development, but we should add it here anyway-->
...
- and voc-d (a further measure of lexical diversity).  <!-- AS: let's exclude voc-d? and say HDD is idealized version of voc-d -->

<!-- sm: MLU is not a measure of lexical diversity-->
...
<!-- AS: screenshots for individual apps? -->
...
Gender differences in language production have been found in multiple studies, notably in @eriksson2012differences, a large-scale study that spanned ten languages and found that females have a higher vocabulary than males in early childhood.
A researcher or student in child language acquisition may now investigate this difference with relative ease using `childesr`.
By pulling data from the `transcript_by_speaker` table, a user has access to the the multitude of derived linguistic measures that are often used to evaluate a child's grammatical development.
In this worked example we walk through a sample analysis that probes the gender differences in early grammatical development.
...
Note that we exclusively select the data of children, and exclude their caregivers.
...
In order to make any substantial inferences from our analysis, we begin by filtering languages for which there are a significant number of children. 
...
For this reason, we fit a linear fixed effects model (`mod_data`, shown in the plot below) to remedy these concerns.^[All code and analyses are available at https://github.com/langcog/childes-db-paper] 
The plot below displays the average MTLD scores for various children at different ages, split by gender, with a line corresponding to our mixed effects model.
...
This plot reveals a slight gender difference in linguistic productivity in young children (displaying a moderate female advantage), replicating the results of @eriksson2012differences.
The goal of this analysis was to showcase an example of the the steps involved in using `childesr` for exploring the CHILDES dataset, as well as some of the potential hazards in working with a diverse set of corpora, many of which are longitudinal. 

<!-- SM: not terribly clear what the hazards were: sparsity and autocorrelation? AS: mostly the fact that there are longitudinal studies in here-->
...
For example, currently childes-db operates only on transcript data, without links to the underlying media files; these linkages facilitate deeper analyses of phonology, prosody, social interaction, and many other aspects of acquisition.
