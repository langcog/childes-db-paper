...
Many times a researcher will want to investigate the statistics of a given corpus (i.e the size, number of utterances, number of tokens, etc.) before choosing a target corpus for their analysis.
...
For example, examining the aggregate statistics reveals that coverage peaks at around 30 months.
...
Users can take advantage of the tools developed in the popular `dplyr` package [@R-dplyr], which make manipulating large datasets quick and easy.
...
More recently, this finding has been shown to be robust in ten languages [@eriksson2012differences].
Important questions remain regarding the relative contribution of differences in the language input to boys and girls vs. maturational differences in neural development.
A researcher or student in child language acquisition may now further investigate these differences with relative ease using `childesr`.
By pulling data from the `transcript_by_speaker` table, a user has access to the multitude of derived linguistic measures that are often used to evaluate a child's grammatical development.
...
This call pulls data from all collections and corpora, including several languages for which there are very sparse data.
In order to make any substantial inferences from our analysis, we begin by filtering out languages with an insufficient number of children. 
We also constrain our analysis to children under the age of four years. 
...
We start by filtering to those children for which a sex was defined, who speak a language in our subset, and are in the appropriate age range.
...
The data contained in CHILDES is populated from a diverse array of studies refelcting varying circumstances of data collection.
...
To rememdy these concerns, we fit a linear mixed effects model with gender * age (treated as a quadratic predictor) as fixed effects, child identity as a random intercept, and age + gender by language as a random slope.^[All code and analyses are available at https://github.com/langcog/childes-db-paper] 
<!--  mod <- lmer(measure ~ target_child_sex * poly(age_years, 2) - 1 + 
                    (1 | target_child_id) +
                    (target_child_sex + age_years | language), 
  data = data) -->
<!-- SM: not sure I got the random effects structure right -->
...
The goal of this analysis was to showcase an example of the the steps involved in using `childesr` for exploring the CHILDES dataset, as well as some of the potential hazards -- sparsity and non-independance-- that emerge in working with a diverse set of corpora, many of which are longitudinal. 
