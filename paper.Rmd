...
Focusing on language learning in naturalistic contexts also reveals that children have in many cases productive and receptive abilities exceeding those demonstrated in experimental contexts. Often, children's most revealing and sophisticated uses of language emerge in naturalistic play.

While corpora of early language acquisition are extremely useful, creating such corpora requires significant resources.
Collecting and transcribing audio and video is costly and extremely time consuming--even orthographic transcription (i.e., transcriptions with minimal phonetic detail) can take ten times the duration of the original recording to transcribe [@macwhinney2000].
...
CHILDES archives tens of thousands of transcripts and associated media across 20+ languages, making it a critical resource for characterizing both children’s early productive language use and their language environment.
...
data <- stats %>%
  ...
  filter(age_years < max_age, target_child_id != 9387)
```
